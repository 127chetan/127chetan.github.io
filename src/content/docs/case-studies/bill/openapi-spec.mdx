...
Writing a YAML at that scale is not just a documentation problem. It is an engineering problem! A single incorrect indentation in a 1000-line YAML block is nearly impossible to debug by eye. At the time, I built a toolchain to catch errors in real time.
