...
My first complete draft of the BILL v2 API OpenAPI spec was ~178,000 lines.

Writing a YAML file at that scale is not just a documentation problem. It is an engineering problem! A single incorrect indentation in a 1000-line YAML block is nearly impossible to debug by eye. At the time, I built a toolchain to catch errors in real time.

<CardGrid>
  <Card title="Render with Swagger Editor">
    `editor.swagger.io` rendered the spec as structured output while I was writing it, not after.
  </Card>
