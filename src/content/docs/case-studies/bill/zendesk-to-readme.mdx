...
The BILL v2 API was built by BILL engineering for BILL engineering. When leadership chose to make the API public, BILL v2 would have benefited from a customer-centric API design strategy. Failure responses returned HTTP 200. Each operation (GET, PUT, PATCH, DELETE) was a POST request. Response field names were unhelpful, and many responses ignored values set in the request.
